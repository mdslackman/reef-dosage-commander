...
class AquariumCommanderPro:
    def __init__(self, root):
        self.root = root
        self.root.title("Aquarium Commander Pro v0.18.6")
        ...
        # Maintenance StringVars (Prevents AttributeError)
    ...
    def update_steps(self, *a):
        ...
        if brand in self.test_instructions and p in self.test_instructions[brand]:
            for txt, sec in self.test_instructions[brand][p]:
                ...
                if sec > 0:
                    btn = tk.Button(r, text=f"⏲ {sec}s", command=lambda b=None, s=sec: None)
    ...
    def calc_dose(self):
        try:
            ...
            self.res_lbl.config(text=f"Check logic for dosing strength", fg="red")
