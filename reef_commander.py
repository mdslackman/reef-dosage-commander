...
class AquariumCommanderPro:
    def __init__(self, root):
        self.root = root
        self.root.title("Aquarium Commander Pro v0.11.9")
        ...
        self.vol_var = tk.StringVar(value="") # Empty by default for safety
    ...
    def build_calc_tab(self):
        ...
        # --- TOP SECTION: TANK VOLUME & pH ---
        top_frame = tk.LabelFrame(f, text=" System Safety Monitor ", padx=10, pady=10)
        top_frame.grid(row=0, columnspan=3, sticky="ew", pady=(0, 20))
        
        tk.Label(top_frame, text="Tank Volume (Gal):", font=("Arial", 10, "bold")).grid(row=0, column=0)
        self.vol_ent = tk.Entry(top_frame, textvariable=self.vol_var, width=10, bg="#fffde7")
        self.vol_ent.grid(row=0, column=1, padx=5)
        
        tk.Label(top_frame, text="System pH:", font=("Arial", 10, "bold")).grid(row=0, column=2, padx=(20, 5))
        tk.Label(top_frame, textvariable=self.live_ph, fg="#c0392b", font=("Arial", 16, "bold")).grid(row=0, column=3)
        ...
        tk.Label(f, text="Measurement Unit:").grid(row=3, column=0, sticky="w")
        ...
        tk.Label(f, text="Manual pH Adj:").grid(row=6, column=0, sticky="w")
        self.ph_ent = tk.Entry(f, textvariable=self.live_ph); self.ph_ent.grid(row=6, column=1, pady=5, sticky="ew")
        ...
        tk.Button(f, text="RUN SAFETY CALCULATION", command=self.perform_calc, bg="#2980b9", fg="white", font=("Arial", 10, "bold")).grid(row=8, columnspan=3, pady=20, sticky="ew")
        self.res_lbl = tk.Label(f, text="Enter Volume to Begin", font=("Consolas", 11, "bold"), wraplength=450, justify="center")
    ...
    def build_maint_tab(self):
        ...
        tk.Button(f, text="SAVE TO HISTORY (STAMPS DATE)", command=self.save_maint, bg="#8e44ad", fg="white", font=("Arial", 10, "bold")).pack(pady=15)
    ...
    def save_maint(self):
        ...
        for name, val in data.items():
            if val:
                self.save_to_csv(d, "Test", name, val); logged = True
        if logged:
            messagebox.showinfo("Success", f"Logged at {d}")
    ...
    def perform_calc(self):
        if not self.vol_var.get():
            messagebox.showwarning("Safety Warning", "You MUST enter Tank Volume before calculating.")
            return
        try:
    ...
    def refresh_logs(self):
        if os.path.exists(LOG_FILE):
            with open(LOG_FILE, "r") as f:
                ...
                self.txt.delete("1.0", tk.END); self.txt.insert(tk.END, content)
