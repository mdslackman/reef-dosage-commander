...
class AquariumCommanderPro:
    def __init__(self, root):
        ...
        self.build_calc_tab(); self.build_maint_tab(); self.build_log_tab(); self.update_param_selection()
    ...
    def build_calc_tab(self):
        ...
        tk.Label(ph_frame, textvariable=self.live_ph, fg="#f1c40f", bg="#2c3e50", font=("Arial", 24, "bold")).pack()

        self.p_var = tk.StringVar(value="Alkalinity"); self.u_var = tk.StringVar(value="dKH")
        self.b_var = tk.StringVar(); self.dyn_u = tk.StringVar(value="dKH")
        ...
        tk.Label(f, text="Manual pH Update:").grid(row=5, column=0, sticky="w")
        ...
        self.res_lbl = tk.Label(f, text="", font=("Consolas", 11, "bold"), wraplength=450); self.res_lbl.grid(row=8, columnspan=3)
    ...
    def build_maint_tab(self):
        ...
        tk.Label(grid, text="Alkalinity:").grid(row=0, column=0); self.m_alk = tk.Entry(grid); self.m_alk.grid(row=0, column=1)
        tk.Label(grid, text="Calcium:").grid(row=1, column=0); self.m_cal = tk.Entry(grid); self.m_cal.grid(row=1, column=1)
        tk.Label(grid, text="Magnesium:").grid(row=2, column=0); self.m_mag = tk.Entry(grid); self.m_mag.grid(row=2, column=1)
        tk.Label(grid, text="pH:").grid(row=3, column=0); self.m_ph_log = tk.Entry(grid, textvariable=self.live_ph); self.m_ph_log.grid(row=3, column=1)
        
        tk.Button(f, text="SAVE TO HISTORY", command=self.save_maint, bg="#8e44ad", fg="white").pack(pady=10)
        
        tk.Label(f, text="TANK SETTINGS", font=("Arial", 10, "bold")).pack(pady=(20, 5))
    ...
    def save_maint(self):
        ...
        if self.m_alk.get(): self.save_to_csv(d, "Test", "Alk", self.m_alk.get())
        if self.m_cal.get(): self.save_to_csv(d, "Test", "Cal", self.m_cal.get())
        if self.m_mag.get(): self.save_to_csv(d, "Test", "Mag", self.m_mag.get())
        if self.live_ph.get(): self.save_to_csv(d, "Test", "pH", self.live_ph.get())
        messagebox.showinfo("Success", "Logged to history.")
    ...
    def perform_calc(self):
        try:
            ...
            # --- AUTO PPM LOGIC ---
            # If current value is high (e.g. 71+), assume PPM and adjust strength
            if p == "Alkalinity" and curr > 20: strength *= 17.86
            
            total_ml = ((targ - curr) * vol) / strength
            limit = self.safety_limits[p]
            if p == "Alkalinity" and curr > 20: limit *= 17.86
            ...
            msg = f"TOTAL: {total_ml:.1f} mL\nDaily: {total_ml/days:.1f} mL ({days} days)"
            if ph_val >= 8.45: msg += f"\n⚠️ HIGH pH: {ph_val}\nSlowing dose for safety."
            
            self.res_lbl.config(text=msg, fg="#c0392b" if ph_val >= 8.45 else "#2980b9")
