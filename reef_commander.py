...
class AquariumCommanderPro:
    def __init__(self, root):
        self.root = root
        self.root.title("Aquarium Commander Pro v0.19.3")
    ...
    def update_steps(self, *a):
        ...
        if brand in self.test_instructions and p in self.test_instructions[brand]:
            for txt, sec in self.test_instructions[brand][p]:
                ...
                if sec > 0:
                    btn = tk.Button(r, text=f"⏲ {sec}s", command=lambda b=None, s=sec: self.run_timer(s))
                    btn.pack(side="right")

    def run_timer(self, seconds):
        if seconds > 0:
            self.root.after(1000, lambda: self.run_timer(seconds-1))
        else: messagebox.showinfo("Timer", "Step Complete!")
    ...
    def refresh_graphs(self):
        ...
        try:
            ...
            for i, (p, r) in enumerate(self.ranges.items()):
                ...
                axes[i].axhspan(low, high, color='green', alpha=0.15) # RESTORED GREEN ZONE
                axes[i].axhline(target, color='red', linestyle='--', alpha=0.5) # RESTORED TARGET LINE
    ...
    def show_context_menu(self, e):
        ...
        if i:
            ...
            m.add_command(label="Delete", command=self.delete_entry); m.post(e.x_root, e.y_root)
