...
class ReeferMadness:
    def __init__(self, root):
        self.root = root
        self.root.title("Reefer Madness v0.21.1 - Bulletproof")
        ...
        tk.Label(header, text="REEFER MADNESS v0.21.1", bg="#2c3e50", fg="#3498db", font=("Arial", 10, "bold")).pack(side="left", padx=20)
    ...
    def build_maint(self):
        ...
        tk.Label(u_row, text="Alk Unit:").pack(side="left")
    ...
    def build_history(self):
        ...
        test_f = ttk.LabelFrame(f, text=" Manual Entry ", padding=10); test_f.pack(fill="x", padx=10, pady=5)
        ...
        tk.Button(test_f, text="DELETE SELECTED", command=self.delete_entry, bg="#c0392b", fg="white").pack(side="right", padx=10)
    ...
    def build_trends(self):
        ...
        tk.Button(f, text="REFRESH GRAPHS", command=self.refresh_graphs, bg="#2c3e50", fg="white").pack(pady=5); self.refresh_graphs()

    def refresh_graphs(self):
        for w in self.t_canv.winfo_children(): w.destroy()
        try:
            if not os.path.exists(self.log_file) or os.stat(self.log_file).st_size < 50: 
                tk.Label(self.t_canv, text="NO DATA LOGGED YET", font=("Arial", 14)).pack(pady=50); return
    ...
    def refresh_history_table(self):
        ...
        if os.path.exists(self.log_file) and os.stat(self.log_file).st_size > 50:
            df = pd.read_csv(self.log_file)
            # Re-index to ensure deletion matches row number
