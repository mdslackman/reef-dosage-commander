...
class AquariumCommanderPro:
    def __init__(self, root):
        self.root = root
        self.root.title("Aquarium Commander Pro v0.19.4")
        ...
        self.alk_u_var = tk.StringVar(value="ppm") # Default to PPM as per your setup
    ...
    def build_dosage(self):
        ...
        tk.Label(r1, text="Product:").pack(side="left")
        self.b_cb = ttk.Combobox(r1, textvariable=self.b_var, state="readonly"); self.b_cb.pack(side="left", padx=5)
        self.custom_pane = ttk.Frame(r1); tk.Label(self.custom_pane, text="Strength:").pack(side="left")
    ...
    def calc_dose(self):
        try:
            ...
            # RESULT: (Gap * (Liters / 100)) / Strength
    ...
    def build_maint(self):
        ...
        # Unit switcher for Alkalinity in Maintenance
        unit_f = ttk.Frame(f); unit_f.pack(fill="x", pady=5)
        tk.Label(unit_f, text="Alk Unit:").pack(side="left")
        ttk.Radiobutton(unit_f, text="dKH", variable=self.alk_u_var, value="dKH").pack(side="left", padx=5)
        ttk.Radiobutton(unit_f, text="PPM", variable=self.alk_u_var, value="ppm").pack(side="left", padx=5)
    ...
    def run_timer(self, btn, seconds):
        if seconds > 0:
            ...
            btn.config(text="DONE", state="normal", bg="lightgreen")
            messagebox.showinfo("Timer", "Time is up!")

    def build_trends(self):
        f = self.tabs["Trends"]
        btn_f = ttk.Frame(f); btn_f.pack(fill="x")
        tk.Button(btn_f, text="🔄 REFRESH GRAPHS", command=self.refresh_graphs).pack(pady=5)
    ...
    def refresh_graphs(self):
        ...
        try:
            ...
            for i, (p, r) in enumerate(self.ranges.items()):
                ...
                axes[i].axhspan(low, high, color='green', alpha=0.15)
