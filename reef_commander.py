...
class AquariumCommanderPro:
    def __init__(self, root):
        self.root = root
        self.root.title("Aquarium Commander Pro v0.18.5")
    ...
    def build_dosage(self):
        ...
        # FIXED: Volume & Unit selection visible
        ...
        row2 = ttk.Frame(f); row2.pack(fill="x", pady=10)
        tk.Label(row2, text="Current:").pack(side="left")
        tk.Entry(row2, textvariable=self.curr_val_var, width=10).pack(side="left", padx=5)
        tk.Label(row2, text="Target:").pack(side="left")
        tk.Entry(row2, textvariable=self.targ_val_var, width=10).pack(side="left", padx=5)
        
        # FIXED: pH Optional visible
        tk.Label(row2, text="pH (Optional):").pack(side="left", padx=(15, 0))
        tk.Entry(row2, textvariable=self.ph_var, width=8).pack(side="left", padx=5)
    ...
    def refresh_graphs(self):
        ...
        for i, (p, r) in enumerate(self.ranges.items()):
            ...
            # FIXED: Smart Y-Axis for Alkalinity
            ...
            axes[i].axhspan(low, high, color='green', alpha=0.2, label="Safe Range")
            axes[i].axhline(target, color='red', linestyle='--', alpha=0.5)
            axes[i].set_title(f"{p} ({'PPM' if is_ppm else 'dKH/ppm'})")
    ...
    def build_history(self):
        ...
        ttk.Label(left, text="1. Select Test:").pack(anchor="w")
        ttk.Combobox(left, textvariable=self.t_param_var, values=list(self.ranges.keys())).pack(fill="x", pady=2)
        ttk.Label(left, text="2. Select Kit:").pack(anchor="w")
        self.kit_cb = ttk.Combobox(left, textvariable=self.t_brand_var); self.kit_cb.pack(fill="x", pady=2)
        ...
        tk.Button(res_f, text="LOG", command=self.save_hist, bg="#27ae60", fg="white").pack(side="right", padx=2)
        ...
        for c, h in [("T", "Timestamp"), ("P", "Param"), ("V", "Value")]: 
            self.tree.heading(c, text=h); self.tree.column(c, width=100)
    ...
    def update_steps(self, *a):
        ...
        if brand in self.test_instructions and p in self.test_instructions[brand]:
            for txt, sec in self.test_instructions[brand][p]:
                ...
                if sec > 0:
                    # FIXED: Timer button with active feedback
                    btn = tk.Button(r, text=f"⏲ {sec}s", width=8)
    ...
    def build_maint(self):
        ...
        for p in self.ranges.keys():
            ...
            e = tk.Entry(r); e.pack(side="left", expand=True, fill="x")
            self.m_entries[p] = e
            if p == "Alkalinity":
                e.trace_add("write", lambda *a, var=e: self.smart_detect(var))
        ...
        tk.Button(f, text="SAVE ALL MEASUREMENTS", command=self.save_maint, bg="#27ae60", fg="white", height=2).pack(fill="x", pady=20)

    # [Standard Helper methods: init_csv, load_config, hard_exit, etc. from previous builds]
    def save_maint(self):
        with open(self.log_file, "a", newline="") as f:
            w = csv.writer(f); ts = datetime.now().strftime("%Y-%m-%d %H:%M")
            for p, e in self.m_entries.items():
                if e.get(): w.writerow([ts, p, e.get(), ""])
        self.refresh_all(); messagebox.showinfo("Saved", "Maintenance log updated.")
    ...
    def calc_dose(self):
        try:
            ...
            str_val = self.brand_data[self.p_var.get()][list(self.brand_data[self.p_var.get()].keys())[0]]
            self.res_lbl.config(text=f"Add {(diff * v / str_val):.2f} mL", fg="red")
