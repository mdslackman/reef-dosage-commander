...
class AquariumCommanderPro:
    def __init__(self, root):
        self.root = root
        self.root.title("Aquarium Commander Pro v0.18.0")
        ...
        self.ranges = {
            "Alkalinity": {"target": 8.5, "low": 7.5, "high": 9.5, "units": "dKH"},
            "Calcium": {"target": 420, "low": 380, "high": 460, "units": "ppm"},
    ...
    def calc_dose(self):
        try:
            ...
            if diff <= 0: self.res_lbl.config(text="Optimal Level", fg="green"); return
            str_val = self.brand_data[self.p_var.get()][self.b_var.get()]
            self.res_lbl.config(text=f"Dose: {(diff * v / str_val):.2f} mL", fg="red")
        except: self.res_lbl.config(text="Error", fg="red")
    ...
    def build_maint(self):
        ...
        tk.Label(r, text="Alkalinity", width=12).pack(side="left")
        e = tk.Entry(r); e.pack(side="left", expand=True, fill="x")
        self.m_entries["Alkalinity"] = e
        ...
        for p in ["Calcium", "Magnesium", "Nitrate", "Phosphate"]:
            r = ttk.Frame(f); r.pack(fill="x", pady=2)
            tk.Label(r, text=p, width=12).pack(side="left")
            e = tk.Entry(r); e.pack(side="left", expand=True, fill="x")
            self.m_entries[p] = e
        tk.Button(f, text="LOG ALL", command=self.save_maint, bg="#27ae60", fg="white").pack(fill="x", pady=10)
    ...
    def refresh_graphs(self):
        ...
        for i, (p, r) in enumerate(self.ranges.items()):
            ...
            axes[i].plot(subset['Timestamp'], subset['Value'], marker='o', color='black', zorder=5)
            # Safety Zones
            axes[i].axhspan(r['low'], r['high'], color='green', alpha=0.2)
            axes[i].axhline(r['target'], color='blue', linestyle='--', alpha=0.6)
            axes[i].set_title(f"{p} Target: {r['target']}")
            axes[i].grid(True, alpha=0.2)

        FigureCanvasTkAgg(fig, master=self.t_canv).get_tk_widget().pack(fill="both")
    ...
    def build_history(self):
        ...
        tk.Button(res_f, text="SAVE", command=self.save_hist).pack(side="right")
        ...
        for c in [("T", "Time"), ("P", "Param"), ("V", "Value")]: self.tree.heading(c[0], text=c[1])
    ...
    def update_steps(self, *a):
        ...
        if brand in self.test_instructions and p in self.test_instructions[brand]:
            for txt, sec in self.test_instructions[brand][p]:
                r = ttk.Frame(self.step_f); r.pack(fill="x")
                tk.Checkbutton(r, text=txt).pack(side="left")
                if sec > 0: tk.Button(r, text=f"⏲ {sec}s").pack(side="right")
    ...
    def delete_entry(self):
        ...
        df = pd.read_csv(self.log_file)
        # Filter out the specific row (approx match on timestamp and param)
        df = df[~((df['Timestamp'] == selected[0]) & (df['Parameter'] == selected[1]))]
        df.to_csv(self.log_file, index=False)
        self.refresh_all()

    def edit_entry(self):
        new_val = tk.simpledialog.askfloat("Edit", "Enter new value:")
        if new_val is not None:
            selected = self.tree.item(self.tree.selection())['values']
            df = pd.read_csv(self.log_file)
            df.loc[(df['Timestamp'] == selected[0]) & (df['Parameter'] == selected[1]), 'Value'] = new_val
            df.to_csv(self.log_file, index=False)
            self.refresh_all()
    ...
    def refresh_all(self):
        self.refresh_history_table(); self.refresh_graphs()
